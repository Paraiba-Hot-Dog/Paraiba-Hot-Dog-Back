"""Endpoints para gestao de Sobre Nos e carrossel de imagens."""
...
router = APIRouter()
compat_router = APIRouter(prefix="/institucional", tags=["institucional"])
...
# ---------------------------------------------------------------------------
# Endpoints de Texto e Estatisticas (/sobre-nos)
# ---------------------------------------------------------------------------


@router.get("", response_model=SobreNosRead)
@router.get("/", response_model=SobreNosRead, include_in_schema=False)
def obter_sobre_nos(db: Session = Depends(get_db)):
    """Retorna o texto institucional e os cards de estatisticas. Publico."""
...
def atualizar_sobre_nos(data: SobreNosUpdate, db: Session = Depends(get_db)):
    """Atualiza o texto e os cards de estatisticas. Requer role administrador."""
    return repository.atualizar_sobre_nos(db, data)


# Compatibilidade legada (/institucional/sobre-nos)
@compat_router.get("/sobre-nos", response_model=SobreNosRead, include_in_schema=False)
def obter_sobre_nos_legado(db: Session = Depends(get_db)):
    """Alias retrocompativel para leitura institucional."""
    return repository.obter_sobre_nos(db)
...
def atualizar_sobre_nos_legado(data: SobreNosUpdate, db: Session = Depends(get_db)):
    """Alias retrocompativel para edicao institucional."""
    return repository.atualizar_sobre_nos(db, data)


# ---------------------------------------------------------------------------
# Endpoints de Carrossel de Imagens (/sobre-nos/imagens)
# ---------------------------------------------------------------------------


@router.get("/imagens", response_model=list[SobreNosImagemRead])
def listar_imagens(db: Session = Depends(get_db)):
    """Lista as imagens do carrossel Sobre Nos ordenadas. Publico."""
...
async def criar_imagem(
    ...
    try:
        return repository.criar_imagem(
            db, SobreNosImagemCreate(imagem_url=imagem_url, posicao=posicao)
        )
    except Exception:
...
def atualizar_imagem(
    imagem_id: int, data: SobreNosImagemUpdate, db: Session = Depends(get_db)
):
    """Atualiza ordem e/ou posicao de uma imagem existente. Requer role administrador."""
